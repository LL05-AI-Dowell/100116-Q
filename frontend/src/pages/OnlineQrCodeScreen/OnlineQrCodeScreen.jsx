...
const QrCodeScreen = () => {
  ...
  useEffect(() => {
    const getTicketLink = async () => {
      //ticket_link is not present in api
      try {
        const response = await fetch(
          `https://www.q.uxlivinglab.online/api/v3/user-services/?type=retrieve_user&workspace_id=${workspaceId}`,
          {
            method: "GET",
            headers: {
  ...
  const createTicket = () => {
    if (!linkId) alert("Ticket Link is not present");
    if (socket) {
            ...
            <div className="cursor-pointer min-h-[50px] w-max fixed right-4 bottom-[200px] rounded flex items-center justify-center">
              <button onClick={showChat}>
                ...
                <svg
                  ...
                  strokeWidth="0"
                  viewBox="0 0 16 16"
                  className="ml-1.5 w-10 h-10 text-green-700"
                  height="1em"
                  width="1em"
                ...
                <>
                  ...
                  >
                    <div className="flex flex-row items-end justify-between h-52">
                      <button
                        className="p-2 bg-red-400 rounded-xl text-blue-700 font-medium"
                        onClick={() => {
                          createTicket();
                        }}
                      >
